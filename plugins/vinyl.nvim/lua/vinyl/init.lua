...
local highlights = require("vinyl.highlights")
local debug = require("vinyl.debug")
local ui = require("vinyl.ui")

-- Backends
local apple_backend = require("vinyl.backends.apple")
local spotify_backend = require("vinyl.backends.spotify")

-- Spotify auth
local spotify_auth = require("vinyl.spotify.auth")
local spotify_api = require("vinyl.spotify.api")
...
local function auto_detect_backend()
	...
	elseif preferred == "apple" then
		-- Try to use Apple Music if it was preferred
		local apple = apple_backend.new()
		if apple.available() and apple.init({}) then
			return apple
		end
		-- Preference was apple but it's not available, fall through to auto-detect
	end
...
function M.get_backend()
	if not current_backend then
		...
		if not current_backend then
			...
			msg = msg .. "- Spotify: Run :lua require('vinyl').spotify_login()"
...
function M.use_backend(backend_name)
	...
	elseif backend_name == "apple" then
		local backend = apple_backend.new()
		if backend.available() and backend.init({}) then
			current_backend = backend
			spotify_state.save_backend_preference("apple")
		...
		vim.ui.input({
			...
			vim.ui.input({
				...
				vim.ui.input({
					...
					if not port then
						vim.notify(
							"Invalid redirect URI - must include port (e.g., http://localhost:8888/callback)",
					...
					local oauth_server = require("vinyl.spotify.oauth_server")
