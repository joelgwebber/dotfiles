...
local api = require("vinyl.spotify.api")
...
function M.new()
	...
	function backend.init(config)
		if not backend.available() then
			vim.notify("Spotify not authenticated. Run :lua require('vinyl').spotify_login()", vim.log.levels.WARN)
			return false
		end
		return true
	end
	...
	function backend.get_state_async(callback)
		api.get("/me/player", {}, function(response, err)
			...
			local state = {
				...
				album_artist = item.album
						and item.album.artists
						and item.album.artists[1]
						and item.album.artists[1].name
					or nil,
	...
	function backend.play_artist(artist_name, callback)
		...
		api.get(search_endpoint, {}, function(search_response, search_err)
			if search_err then
				if callback then
					callback(false, search_err)
				end
				return
			end

			if
				not search_response.artists
				or not search_response.artists.items
				or #search_response.artists.items == 0
			then
				if callback then
			...
			api.get(top_tracks_endpoint, {}, function(tracks_response, tracks_err)
				...
				if not tracks_response.tracks or #tracks_response.tracks == 0 then
					if callback then
						callback(false, "No tracks found for artist")
					end
					return
				end
			...
			api.get("/me/player/queue", {}, function(response, err)
				...
				if response.queue and response.queue ~= vim.NIL then
					for i, item in ipairs(response.queue) do
						...
						-- Extract artwork URL for this track (same logic as get_state_async)
						local artwork_url = nil
						if item.album and item.album.images and #item.album.images > 0 then
							local images = item.album.images

							-- Try to find a small image for queue thumbnails (< 300px)
							for _, img in ipairs(images) do
								if img.width and img.width < 300 then
									artwork_url = img.url
									break
								end
							end

							-- Fallback to smallest image if no small one found
							if not artwork_url and #images > 0 then
								artwork_url = images[#images].url
							end
						end
						...
						table.insert(upcoming_tracks, {
							...
							artist = table.concat(artists, ", "),
							album = item.album and item.album.name or nil,
							artwork_url = artwork_url, -- Include artwork URL for Spotify
						})
					end
				end
