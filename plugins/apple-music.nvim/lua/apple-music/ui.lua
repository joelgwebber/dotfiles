...
local function format_stats_line(state)
  ...
  if state.played_count and state.played_count > 0 then
    table.insert(parts, string.format('Played %d time%s', state.played_count, state.played_count == 1 and '' or 's'))
  end
...
local function render_ui()
  ...
  player.get_state_async(function(state)
    ...
    -- Display artwork if enabled and available
    if state.artwork_count and state.artwork_count > 0 and config.options.artwork.enabled then
      -- Display artwork at row 1, col 2 (inside the window)
      artwork.display(M.buf, 1, 2)
    else
      artwork.clear()
    end
  end)
...
function M.open()
  ...
  vim.api.nvim_buf_set_keymap(M.buf, 'n', 'p', ':lua require("apple-music.player").play_pause()<CR>', { silent = true })
  vim.api.nvim_buf_set_keymap(M.buf, 'n', 'n', ':lua require("apple-music.player").next_track()<CR>', { silent = true })
  vim.api.nvim_buf_set_keymap(M.buf, 'n', 'N', ':lua require("apple-music.player").previous_track()<CR>', { silent = true })
  vim.api.nvim_buf_set_keymap(M.buf, 'n', '=', ':lua require("apple-music.player").increase_volume()<CR>', { silent = true })
  vim.api.nvim_buf_set_keymap(M.buf, 'n', '-', ':lua require("apple-music.player").decrease_volume()<CR>', { silent = true })
  vim.api.nvim_buf_set_keymap(M.buf, 'n', 's', ':lua require("apple-music.player").toggle_shuffle()<CR>', { silent = true })

  render_ui()

  if not M.timer then
    M.timer = vim.loop.new_timer()
    M.timer:start(0, config.options.update_interval, vim.schedule_wrap(function()
      if M.win and vim.api.nvim_win_is_valid(M.win) then
        render_ui()
      end
    end))
